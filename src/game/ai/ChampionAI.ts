...
import { BeloteAI, AIContext, AIDecision } from './BeloteAI'
import { AILevel, Suit, Team } from '../models/Types'
/**
...
export class ChampionAI extends BeloteAI {
  ...
  private analyzeGameState(context: AIContext) {
    ...
    return {
      ...
      isNearEnd: Math.max(nsScore, ewScore) > (target ?? 1000) * 0.8,
    }
  }
  ...
  private generateChampionReasoning(
    ...
    if (trickInfo.isEmpty) {
      if (cardInfo.isTrump && cardInfo.isJackOfTrump) {
        return 'Entame mątresse au valet d\'atout'
      } else if (cardInfo.isHighValue) {
        return 'Entame forcing calcul̩e'
      } else {
    ...
    if (trickInfo.isPartnerWinning) {
      if (cardInfo.points <= 2) {
        return 'D̩fausse parfaite, partenaire mątre'
      } else {
        return 'Soutien strat̩gique optimis̩'
      }
    }
    ...
    if (this.canCardWinTrick(card, context)) {
      ...
      if (efficiency > 0.2) {
        ...
        return 'Prise n̩cessaire calcul̩e'
      }
    }

    return 'Coup de champion analys̩'
  }
}
